...
* [More ZCTA demographics (U.S. Census Bureau, 2019 ACS 5-year estimates)](https://www.census.gov/acs/www/data/data-tables-and-tools/data-profiles/)
...
```{r vaccination data}
vax_by_zip <-
  read_csv("https://data.cityofchicago.org/api/views/553k-3xzc/rows.csv?accessType=DOWNLOAD") %>%
...
# start of x-axis
date_cutoff <- mdy("3/21/20")

vaxdatecutoff <- mdy("12/13/20")
...
tocluster <- joined_df %>%
  ...
  mutate(vax_group = cut_interval(cumulative_pct_first, n_groups))
...
ggsave("cumulativevaxallzip.png", cumulativevaxallzip, height = 6, width = 9)
...
vaxcumulative <- ggplot(data = filter(to_graph, week_st >= vaxdatecutoff), 
...
ggsave(paste0("vaccinationscumulative_", n_groups, "groups.png"), vaxcumulative, height = 6, width = 9)
...
vaxweekly <- ggplot(data = filter(to_graph, week_st >= vaxdatecutoff), aes(x = week_st, y = first_dose_weekly)) +
  aesthetics +
  labs(x = "Week", y = "Weekly First Doses/100,000 Population", title = "Trends in Weekly  Vaccination")

vaxweekly

ggsave(paste0("vaccinationsweekly_", n_groups, "groups.png"), vaxweekly, height = 5, width = 9)
...
mortalitycumulative <-
  ggplot(data = to_graph, aes(x = week_st, y = deaths_cumulative)) +
  aesthetics +
  labs(
    x = "\nWeek",
...
ggsave(paste0("mortalitycumulative_", n_groups, "groups.png"),
       mortalitycumulative,
       height = 5,
       width = 9)
...
mortalityweekly <-
  ggplot(data = to_graph, aes(x = week_st, y = death_rate_weekly)) +
  aesthetics +
  geom_linerange(aes(ymin = death_rate_weekly-deathratesd, ymax = death_rate_weekly+deathratesd, color = vax_group)) +
  labs(
    x = "Week",
    y = "Weekly Mortality Rate (per 100,000)",
    title = "Trends in Weekly COVID-19 Mortality Rate after Vaccination")

mortalityweekly

ggsave(paste0("mortalityweekly_", n_groups, "groups.png"),
       mortalityweekly,
       height = 5,
       width = 9)
```
...
rollingweeklymortality <-
  ...
  labs(x = "\nWeek", y = "Deaths per 100,000\n",
    title = "Trends in Weekly COVID-19 Mortality Rate after Vaccination - Rolling Averages")

rollingweeklymortality

ggsave(paste0(k, "wkrollingavg_weeklymortality_", n_groups, "groups.png"), rollingweeklymortality, height = 5, width = 9)

mortrollsd <-   ggplot(data = to_graph, aes(x = week_st, y = mort_roll)) +
  labs(x = "\nWeek", y = "Deaths per 100,000\n",
       title = "Trends in Weekly COVID-19 Mortality Rate after Vaccination - Rolling Averages") +
  aesthetics +
geom_linerange(data = filter(to_graph, is.na(mort_roll) == FALSE), aes(ymin = death_rate_weekly-deathratesd, ymax = death_rate_weekly+deathratesd, color = vax_group))

ggsave("mortrollsd.png", mortrollsd, height = 5, width = 9)
...
mortalitymap <- ggplot() + 
   geom_sf(data = tomapmortality, aes(geometry = geometry, fill = cumulative_deaths)) +
    ...
    scale_color_manual(values = mapcolorscustom) +
    labs(fill = "Deaths during Third Wave", title = "Deaths During 3rd Wave") +
    theme(legend.position = "right", 
        plot.title = element_text(hjust = 0.5),
        legend.title = element_text(size = 12, face = "bold"),
        legend.text = element_text(size = 12),
...
ggsave("thirdwavemortalitymap.png", mortalitymap, height = 7, width = 7)
...
mortalitybar <- to_graph %>%
  ...
  ggplot(aes(x = wave, y = pct_deaths, fill = vax_group)) +
    ...
    geom_text(aes(x = wave, label = paste0(cumulative_deaths_abs, "(", round(pct_deaths), ")")), 
              ...
              size = 6) +
    scale_fill_manual(values = graphcolors) +
    labs(y = "% of Deaths", x = "Wave", title = "Deaths during waves by vaccination group",
...
newcases <- ggplot(data = to_graph, aes(x = week_st, y = case_rate_weekly)) +
  aesthetics +
  labs(x = "\nWeek", y = "Weekly Cases Rate (per 100,000)\n",
    title = "Trends in COVID-19 Cases after Vaccination")

ggsave(paste0("newcasesweekly_", n_groups, "groups.png"), newcases, height = 5, width = 9)
...
ggsave(paste0("newcasesroll_", n_groups, "groups.png"), newcasesroll, height = 5, width = 9)
```
...
mortalitycumulativedid <- mortalitycumulative +
  ...
  geom_line(data = filter(to_did, vax_group == n_labels[1]), 
            ...
            linetype = "dashed") +
  scale_fill_manual(values = c(graphcolors[2], graphcolors[3], graphcolors[1], "#FFFFFF")) +
  scale_color_manual(values = c(graphcolors[2], graphcolors[3], graphcolors[1], "#FF0000")) +
  scale_shape_manual(values = c(graphshapes[2], graphshapes[3], graphshapes[1], 1)) +
  labs(color = group_legend, fill = group_legend, shape = group_legend)
  
mortalitycumulativedid

ggsave(paste0("mortalitycumulativedid_", n_groups, "_groups.png"), mortalitycumulativedid, height = 5, width = 9)
...
## Weekly mortality DID

```{r weekly mortality DID}
mortalityweeklydid <- mortalityweekly +
  # filtered out counterfactuals for high group
  geom_point(data = filter(to_did, vax_group == n_labels[1]), 
             aes(x = week_st, y = absdeath_rate_weekly_counterfactual, 
                 shape = vax_group), color = "red", size = 2, show.legend = FALSE) +
  # filtered out counterfactuals for high group
  geom_line(data = filter(to_did, vax_group == n_labels[1]), 
            aes(x = week_st, y = absdeath_rate_weekly_counterfactual, group = vax_group),
            color = "red", linetype = "dashed") +
  geom_vline(xintercept = mean(DID_st$week_st), color = "red")

# mortalityweeklydid

ggsave(paste0("mortalityweeklydid_", n_groups, "_groups.png"), mortalityweeklydid, height = 5, width = 9)

to_did_low <- to_did %>%
  filter(vax_group == n_labels[1], week_st >= DID_stdate)

# Estimated number of lives saved if low zip codes had gotten high vaccination level using area under curve
# AUC(to_did_low$week, to_did_low$death_rate_weekly, method = "trapezoid") * (mean(to_did_low$population)/100000)
# 
# AUC(to_did_low$week, to_did_low$absdeath_rate_weekly_counterfactual, method = "trapezoid") * (mean(to_did_low$population)/100000)
# 
# to_did_high <- to_did %>%
#   filter(vax_group == n_labels[n_groups], week_st >= DID_stdate)
# 
# AUC(to_did_high$week, to_did_high$death_rate_weekly, method = "trapezoid") * (mean(to_did_high$population)/100000)
```
...
ggsave(paste0("rollingavgmortdid_", n_groups, "_groups.png"), rollingmortdid, height = 5, width = 9)
```
...
# Run Chicago_Vaccinations_SZ.Rmd to line 175 before running this file to import and clean data
...
vaxlevel <- joined_df %>%
  ...
  dplyr::select(zip, population, vaxleveldec, recoveredleveldec)
...
write.csv(model1df, "model1df.csv")

```

```{r model 1 the og}
# Random effects on intercept from zip and week
model1 <- glmer(data = model1df, deaths_weekly ~ (1 + week|zip) + week + wave + wave:vaxleveldec + offset(log(population)), family = poisson(link = "log"))

model1

model1noslope <- glmer(data = model1df, deaths_weekly ~ (1|zip) + week + wave + wave:vaxleveldec + offset(log(population)), family = poisson(link = "log"))
...
Model 1b: 6-week lag on vaccination effect

```{r model1b vax level 6 weeks prior}

model1b <- glmer(data = model1df, deaths_weekly ~ (1|zip) + week + wave + wave:vaxleveldeclag + offset(log(population)), family = poisson(link = "log"))
...
```{r model 2 recovered and aging and IRRS}

# not sure how to account for people who were recovered + vaccinated
# overfitting?

model2 <- glmer(data = model1df, deaths_weekly ~ (1|zip) + week + wave + wave:vaxleveldec + recoveredleveldec + dec65plus + offset(log(population)), family = poisson(link = "log"))

model2

# Wave IRR with median values

paste("Wave IRR with median vaccination level (Model 1 no slope):", exp(fixef(model1noslope)[3] + 
      fixef(model1noslope)[4]*mean(vaxlevel$vaxleveldec)))

paste("Wave IRR with median vaccination level (Model 2):", exp(fixef(model2)[3] + 
      fixef(model2)[6]*mean(vaxlevel$vaxleveldec)))

# Week IRR with median values

paste("Week IRR with median vaccination level (Model 1 no slope):", exp(fixef(model1noslope)[2] + 
      fixef(model1noslope)[4]*mean(vaxlevel$vaxleveldec)))

paste("Week IRR with median vaccination level (Model 2):", exp(fixef(model2)[2] + 
      fixef(model2)[6]*mean(vaxlevel$vaxleveldec)))
...
```{r model 2b 6wk lagged vax effect}

model2b <- glmer(data = model1df, deaths_weekly ~ (1|zip) + week + wave + wave:vaxleveldeclag + recoveredleveldec + dec65plus + offset(log(population)), family = poisson(link = "log"))
...
```{r model1c vax level groups}

model2c <- glmer(data = model1df, deaths_weekly ~ (1|zip) + week + wave + wave:vax_group + recoveredleveldec + dec65plus + offset(log(population)), family = poisson(link = "log"))

model2c

```

```{r parallel slope test}

paralleltrendtest <- glm(data = filter(model1df, wave == 0), deaths_weekly ~ week*vax_group + offset(log(population)), family = poisson(link = "log"))

summary(paralleltrendtest)

linearHypothesis(paralleltrendtest, c("week:vax_group[17.9,28.4] = 0", "week:vax_group(28.4,38.9]"))
...
tab_model(model1noslope, model2,
...
didsim <- simulate(model2, newdata = diddata)
...
model2didplot <- ggplot(data = model2did) +
...
test <- joined_df %>%
  dplyr::group_by(week_st) %>%
  filter(population > pop_limit) %>%
  summarise(deaths_weekly = sum(deaths_weekly), population = sum(population))

ggplot(test) +
  geom_point(aes(x = week_st, y = deaths_weekly)) +
