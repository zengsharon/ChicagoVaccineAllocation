...
popdenominators <- economics_social_by_zip %>%
  dplyr::select(zip, population, pop25over, civiliannoninst) %>%
  left_join(groupsabbr) %>%
  dplyr::group_by(vax_group) %>%
...
table1 <- left_join(table1num, table1pct) %>%
  mutate(Population = popdenominators$population,
...
tochi <- table1num[,1:12] %>%
  dplyr::select(-c(vax_group))
rownames(tochi) <- n_labels
...
table1 <- t(table1) %>%
  cbind(c(NA,
    ...
    #  median age
    kruskal.test(`Median Age (Years)`~vax_group, data = economics_social_by_zip)$p.value,
    # sex
...
colnames(table1) <- append(n_labels, "p")
...
thirdwavedf <- to_graph %>%
   filter(week_st >= thirdwave_st & week_st <= thirdwave_end) %>%
  filter(is.na(mort_roll) == FALSE)

paste("low vax peak:", max(filter(thirdwavedf, vax_group == n_labels[1])$mort_roll))
paste("mid vax peak:", max(filter(thirdwavedf, vax_group == n_labels[2])$mort_roll))
paste("high vax peak:", max(filter(thirdwavedf, vax_group == n_labels[3])$mort_roll))
...
ggplot(data = joined_df) +
  geom_line(aes(x = week_st, y = case_rate_weekly, color = zip), alpha = 0.5)
```
...
lowdid <- to_did %>%
  filter(week_st == thirdwave_end|week_st == thirdwave_st, vax_group == n_labels[1])

highdid <- to_did %>%
  filter(week_st == thirdwave_end|week_st == thirdwave_st, vax_group == n_labels[3])

lowgroupthirdwavedeaths <- ((lowdid$deaths_cumulative[2] - lowdid$deaths_cumulative[1])*lowdid$population/100000)[1]

highgroupthirdwavedeaths <- ((highdid$deaths_cumulative[2] - highdid$deaths_cumulative[1])*highdid$population/100000)[1]

lowgroupthirdwavecounterfactual <- ((lowdid$absdeaths_counterfactual[2] - lowdid$absdeaths_counterfactual[1])*lowdid$population/100000)[1]

didlivessaved <- lowgroupthirdwavedeaths - lowgroupthirdwavecounterfactual

paste("Lives saved if lowest vaccinated zip codes experienced same trend as highly vaccinated:", didlivessaved)
